...
export-env {
    ...
    let virtual_env = r#'/home/unimelb.edu.au/pillaip/STI-Modelling-Project/devMay2026/partnersim-dynet/.venv'#
    let bin = r#'bin'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'partnersim-dynet-py3.11'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'partnersim-dynet-py3.11'#
    }
